...
public class Minesweeper extends Module {
    public Minesweeper() {
        super(MinigamesAddon.CATEGORY, "Minesweeper", "Play Minesweeper from the comfort of your Meteor Client.");
        runInMainMenu = true;
    ...
    private static final String GAME_FOLDER = "minesweeper";
    ...
    public void saveGame(WMinesweeper.SaveState data) {
        ...
        if (!Files.exists(save)) {
            try {
                ...
                MinigamesAddon.LOG.error("Failed to create Minesweeper save file..! Why: {}", err.toString());
            }
        }
        try (Writer writer = Files.newBufferedWriter(save, StandardOpenOption.TRUNCATE_EXISTING)) {
            GSON.toJson(data, writer);
        } catch (Exception err) {
            MinigamesAddon.LOG.error("Minesweeper: {}", err.toString());
        }
    }
    ...
    public void onActivate() {
        ...
        if (!Files.exists(save)) {
            try {
                ...
                error("Failed to create save file§c..!", this.name);
                MinigamesAddon.LOG.error("Failed to create a Minesweeper save file..! Why: {}", err.toString());
            }
        }
        ...
        try {
            ...
            MinigamesAddon.LOG.error("Failed to open Minesweeper screen: {}", err.toString());
            toggle();
        }
    }
    ...
    public void onDeactivate() {
        if (mc.currentScreen instanceof MinesweeperScreen) {
            try {
                ...
                MinigamesAddon.LOG.error("Failed to close Minesweeper screen: {}", err.toString());
                toggle();
            }
        }
    }
