...
import dev.minigames.addon.gui.screens.MeteoritesScreen;
import meteordevelopment.meteorclient.gui.widgets.WWidget;
import dev.minigames.addon.gui.widgets.meteorites.SaveData;
import dev.minigames.addon.gui.widgets.meteorites.FieldSize;
import dev.minigames.addon.gui.widgets.meteorites.HighScore;
...
public class Meteorites extends Module {
    public Meteorites() {
        super(MinigamesAddon.CATEGORY, "Meteorites", "Play Meteorites (an Asteroids-style arcade shooter).");
        runInMainMenu = true;
    ...
    public void saveGame(SaveData data) {
        ...
        if (!Files.exists(save)) {
            try {
                ...
                MinigamesAddon.LOG.error("[Meteorites]: Failed to create save file. {}", err.toString());
            }
        }
        try (Writer writer = Files.newBufferedWriter(save, StandardOpenOption.TRUNCATE_EXISTING)) {
            GSON.toJson(data, writer);
        } catch (Exception err) {
            MinigamesAddon.LOG.error("[Meteorites]: {}", err.toString());
        }
    }
    ...
    public void saveHighScore(HighScore score) {
        ...
        if (!Files.exists(scores)) {
            try {
                Files.createFile(scores);
            } catch (Exception err) {
                MinigamesAddon.LOG.error("[Meteorites]: Failed to create high score file. {}", err.toString());
            }
        }
    ...
    public void onActivate() {
        ...
        try {
            ...
            MinigamesAddon.LOG.error("[Meteorites]: Failed to open meteorites screen. {}", err.toString());
        }
    }
    ...
    public void onDeactivate() {
        ...
        if (mc.currentScreen instanceof MeteoritesScreen) {
            try {
                ...
                MinigamesAddon.LOG.error("[Meteorites]: Failed to close Meteorites screen. {}", err.toString());
            }
        }
    }
